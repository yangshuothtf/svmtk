...
/***********************************************************************/
/* begin yangshuo */
// In VisualStuido's solution explorer, right-click project, -->properties, change "Compile As" to C++
#include <iostream>
//#include <unistd.h> // for usleep(1000);
#include <string>
using namespace std;
extern void load_matrix();
...
{  
  ...
  /* yangshuo*/
  //target  |BT|...|ET| |BS| vectors:location |ES| vectors:value...
  //******* from tree_manager::read_a_vector(), we can see |BV|..|EV| is totally not allowed!!!!!!!  *****
  // 1 	|BT| (NP (NP (DT The)(NNP U.S.))(SBAR (S (VP (VP (VBG finalizing)))))) |ET| 4:1 4328:1 12486:1 12494:1 14235:1 15794:1 21478:1
  load_matrix();
...
//  printf("\nyangshuo: kernel_type: %ld, learn_parm.type: %ld\n\n", kernel_parm.kernel_type, learn_parm.type);
  ...
  /* begin yangshuo */
  printf("docnum=-1 has been counted for %ld times\n", docnum_null_in_advanced_kernels);
  printf("begin free semantic_kernel.\n");
  int j = 0;
  for (i = 0; i < totdoc; i++) {
      if (docs[i].semanticVector == NULL) {
          continue;
      }
      for (j = 0; j < docs[i].semanticVector->num_of_words; j++) {
//          free(docs[i].semanticVector->words[j].wordID);
//          free(docs[i].semanticVector->words[j].location);
          delete[] docs[i].semanticVector->words[j].wordID;
          delete[] docs[i].semanticVector->words[j].location;
          //          delete &(docs[i].semanticVector->words[j]);
      }
      delete[] docs[i].semanticVector->words;
      delete[] docs[i].semanticVector->values;
      /*      for (j = 0; j < docs[i].semanticVector->num_of_words; j++) {
          if (&(docs[i].semanticVector->words[j]) != NULL)
          {
              printf("%d, is not null,%p,%s,%s\n", j, &(docs[i].semanticVector->words[j]), docs[i].semanticVector->words[j].wordID, docs[i].semanticVector->words[j].location);
              fflush(stdout);
          }
      }*/
      delete docs[i].semanticVector;
  }
  printf("begin free matrix for semantic_kernel.\n");
  free_matrix();
  printf("end free semantic_kernel.\n");
  /* end yangshuo */
  for (i = 0; i < totdoc; i++) {
      freeExample(&docs[i]);
  }
