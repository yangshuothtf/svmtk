...
{
//    printf("yangshuo. basic_kernel_not_norm: second_kernel=%d,A.DocID=%d,B.DocID=%d,i=%d,j=%d\n", kernel_parm->second_kernel, a->docnum, b->docnum, i, j);
...
double basic_kernel(KERNEL_PARM *kernel_parm, DOC *a, DOC *b, int i, int j){
		...
		if(a->num_of_vectors <=i)
               ...
               exit(-1);
                ...
                double tmpVal = basic_kernel_not_norm(kernel_parm, a, b, i, j);
                printf("yangshuo: basic_kernel: a.ID=%ld, twonorm_STD=%f, b.ID=%ld, twonorm_STD=%f, tmpVal=%lf, i=%d, j=%d\n", a->docnum, a->vectors[i]->twonorm_STD, b->docnum, b->vectors[i]->twonorm_STD, tmpVal, i,j);
                fflush(stdout);
/*            for linear:
                    a=[1,3,5], b=[2,4,6]
                    a*b = 2+12+30=44
                    a*b/sqrt(aa*bb) = 44/sqrt(35*56)=0.993858
                    a=[1,3,5], b=[6,4,2]
                    a*b = 6+12+10=28
                    a*b/sqrt(aa*bb) = 28/sqrt(35*56)=0.632455
 */
                {
...
//fflush(stdout);
 ...
 //            printf("yangshuo: evaluateNorma: docID=%ld, twonorm_STD=%f, i=%d\n", d->docnum, d->vectors[i]->twonorm_STD, i);
 //            printf ("Norm %f\n",k);
     ...
     /* calculate the kernel function */
{
  switch(kernel_parm->vectorial_approach_standard_kernel) {
   ...
   {            double tmp = (sequence(kernel_parm, a, b));
//   printf("yangshuo, choose_second_kernel()=%lf\n", tmp);
   return tmp; }
...
double advanced_kernels(KERNEL_PARM * kernel_parm, DOC * a, DOC * b){
  ...
  double k1,
         k2;
  double k3; // yangshuo: semantic_kernel
/* TEST
        tmp = (k1*k2);
...
*/
 ...
 switch(kernel_parm->combination_type) {
    ...
    case 'A': /* sum first and second kernels*/
              k1 = choose_tree_kernel(kernel_parm, a, b);
              k2 = choose_second_kernel(kernel_parm, a, b);
              k3 = choose_semantic_kernel(kernel_parm, a, b);
              return k2 + kernel_parm->tree_constant*k1+k3; // yangshuo: semantic_kernel
    case 'X': /* multiply first and second kernels*/
			  k1 = choose_tree_kernel(kernel_parm, a, b);
              k2 = choose_second_kernel(kernel_parm, a, b);
              k3 = choose_semantic_kernel(kernel_parm, a, b);
              return k1*k2*k3;// yangshuo: semantic_kernel
    case 'T': /* only trees */
              k1 = choose_tree_kernel(kernel_parm, a, b);
              return k1;
    case 'V': /* only vectors*/
    ...
    case 'S': /* only semantic*/
        k3 = choose_semantic_kernel(kernel_parm, a, b); // yangshuo: semantic_kernel
        return k3;
    default: printf("Error: Unknown kernel combination for %c\n", kernel_parm->combination_type);
             fflush(stdout);
             exit(1);
   }
}
