...
/* begin yangshuo */
extern int docnum_null_in_advanced_kernels; //count for docnum=-1

typedef struct semanticWord {
    char*    wordID=NULL;	            /* word id, 1,2,3, not like C0026809 */
    char*    location=NULL;              /* word location, like B01.050.150.900.649.313.992.635.505.500 */
} SEMANTIC_WORD;

typedef struct setOfSemanticVectors {
    double           *values=NULL;          /* array of values */
    SEMANTIC_WORD    *words=NULL;      /* The words/values in the document. */
    int num_of_words; 
} SEMANTIC_VECTOR_SET;
...
typedef struct doc {
  ...
  SEMANTIC_VECTOR_SET * semanticVector=NULL; /*  yangshuo: A set of semantic words */
...
/* begin yangshuo */
extern double choose_semantic_kernel(KERNEL_PARM * kernel_parm, DOC * a, DOC * b);
extern void read_semantic_vector_set(char *line, DOC *example, int *pos);
/* end yangshuo */
