...
void svm_learn_classification(DOC *docs, double *classA, long int
			      totdoc, long int totwords, 
			      LEARN_PARM *learn_parm, 
			      KERNEL_PARM *kernel_parm, 
			      KERNEL_CACHE *kernel_cache, MODEL *model)
...
void write_model(char *modelfile, MODEL *model)
{
  ...
  for(i=1;i<model->sv_num;i++) {
    ...
    if (model->supvec[i]->semanticVector != NULL)
    {
        ...
        {
            SEMANTIC_WORD* tmpSemanticWord = NULL;
            fprintf(modelfl, " |BS| ");
            for (j = 0; j < model->supvec[i]->semanticVector->num_of_words; j++) {
                tmpSemanticWord = &(model->supvec[i]->semanticVector->words[j]);
                fprintf(modelfl, "%s:%s ", tmpSemanticWord->wordID, tmpSemanticWord->location);
            }
            fprintf(modelfl, " |ES| ");
        }
    }
