...
//  doc.words = (WORD *)my_malloc(sizeof(WORD)*(max_words_doc+10));
  ...
  while((!feof(docfl)) && fgets(line,(int)lld,docfl)) {
    if(line[0] != '1' && line[0] != '-') continue;  /* line not starts from 1 or -1 */
    doc.docnum = totdoc+1;  
    cout << "call parse_document, docnum=" + to_string(doc.docnum) << endl;
    parse_document(line,&doc,&doc_label,&wnum,max_words_doc,&model.kernel_parm);
    cout << "called parse_document, docnum=" + to_string(doc.docnum) << endl;
//    if (doc_label != 1 || doc_label != -1) continue;//yangshuo, ÆÁ±Î·Ç1£¬-1µÄ¾ä×Ó
    cout << "doc_label=" + to_string(doc_label) << endl;
    totdoc++;
    if(model.kernel_parm.kernel_type == 0) {   /* linear kernel */
  ...
  /* begin yangshuo */
  cout<< "free model.semanticVector" << endl;
/*  for (i = 0; i < totdoc; i++) {
      if (model.supvec[i]->semanticVector == NULL) {
          continue;
      }
      cout << "yangshuo, free model.semanticVector.2" << endl;
      if (model.supvec[i]->semanticVector->num_of_words > 0) {
          for (j = 0; j < model.supvec[i]->semanticVector->num_of_words; j++) {
              delete[] model.supvec[i]->semanticVector->words[j].wordID;
              delete[] model.supvec[i]->semanticVector->words[j].location;
          }
      }
      cout << "yangshuo, free model.semanticVector.3" << endl;
    //  if(model.supvec[i]->semanticVector->words!=NULL)
//      delete[] model.supvec[i]->semanticVector->words;
      cout << "yangshuo, free model.semanticVector.3.1" << endl;
//      delete[] model.supvec[i]->semanticVector->values;
      cout << "yangshuo, free model.semanticVector.3.2" << endl;
      delete model.supvec[i]->semanticVector;
      cout << "yangshuo, free model.semanticVector.4" << endl;
  }*/
  cout << "yangshuo, free matrix" << endl;
  free_matrix();
  /* end yangshuo */
  cout << "yangshuo, free Example" << endl;
...
//   }
  ...
  free(model.supvec);
  free(model.alpha);
  cout << "yangshuo, free model.lin_weights" << endl;
  if(model.kernel_parm.kernel_type == 0) { /* linear kernel */
