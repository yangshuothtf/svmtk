     ...
     /* calculate the kernel function */
{
  ...
  switch(kernel_parm->kernel_type) {
    case 0: /* linear */ 
    {            CFLOAT tmp = (CFLOAT)sprod(a, b);
//    printf("yangshuo, kernel.linear=%f\n", tmp);
    return tmp; }
//            return(CFLOAT)sprod(a, b); 
    ...
    {
        CFLOAT tmp = ((CFLOAT)advanced_kernels(kernel_parm, a, b));
        if (kernel_cache_statistic > 120) {
            if (kernel_cache_statistic % 50000 == 0) {
                printf("%ld, yangshuo, kernel.advanced_kernels, a.ID=%ld,", kernel_cache_statistic, a->docnum);
                if (a->semanticVector != NULL) {
                    printf(" count=%ld, b.ID=%ld,", a->semanticVector->num_of_words, b->docnum);
                }
                else {
                    printf(" count=0, b.ID=%ld,", b->docnum);
                }
                if (b->semanticVector != NULL) {
                    printf(" count=%ld, val=%lf\n", b->semanticVector->num_of_words, tmp);
                }
                else {
                    printf(" count=0, val=%lf\n", b->docnum, tmp);
                }
            }
            return tmp;
        }
        if (kernel_parm->combination_type == 'S') {
            printf("%ld, yangshuo, kernel.advanced_kernels, semantic a.ID=%ld, [", kernel_cache_statistic, a->docnum);
            if (a->semanticVector != NULL) {
                if (a->semanticVector->num_of_words != 0) {
                    for (int i = 0; i < a->semanticVector->num_of_words; i++) {
                        printf("**%s:%s, ", a->semanticVector->words[i].wordID, a->semanticVector->words[i].location);
                    }
                }
                printf("] count=%d, ||| b.ID=%ld, [", a->semanticVector->num_of_words, b->docnum);
            }
            else
            {
                printf("] count=0, ||| b.ID=%ld, [", b->docnum);
            ...
            if (b->semanticVector != NULL) {
                if (b->semanticVector->num_of_words != 0) {
                    for (int i = 0; i < b->semanticVector->num_of_words; i++) {
                        printf("**%s:%s, ", b->semanticVector->words[i].wordID, b->semanticVector->words[i].location);
                    }
                }
            ...
            {
                printf("] count=0, val=%f\n", tmp);
            }
        }
        else if (kernel_parm->combination_type == 'V') {
            int counta = 0, countb = 0;
            if (a->num_of_vectors != 0) {
                while (a->vectors[0]->words[counta].wnum != 0) {
                    counta++;
                }
            }
            if (b->num_of_vectors != 0) {
                while (b->vectors[0]->words[countb].wnum != 0) {
                    countb++;
                }
            }
            printf("%ld, yangshuo, kernel.advanced_kernels, vector a.ID=%ld, [", kernel_cache_statistic, a->docnum);
            if (counta != 0) {
                for (int i = 0; i < counta; i++) {
                    printf("%d,", a->vectors[0]->words[i].wnum);
                }
            }
            printf("] count=%d, ||| b.ID=%ld, [", counta, b->docnum);
            if (countb != 0) {
                for (int i = 0; i < countb; i++) {
                    printf("%d,", b->vectors[0]->words[i].wnum);
                }
            }
            printf("] count=%d, val=%f\n", countb, tmp);
        }
        else {
            printf("%ld, yangshuo, kernel.advanced_kernels, unknown a=%ld, count=%d,||| b=%ld, count=%ld, val=%f\n", kernel_cache_statistic, a->docnum, a->num_of_vectors,
                b->docnum, b->num_of_vectors, tmp);
        }
...
/* begin yangshuo */
void read_one_line(char *line, DOC *doc, int &pos, long int max_words_doc, long int *totwords, KERNEL_PARM *kernel_parm)
{
  ...
  // writeTreeString((model->supvec[i])->root);
   /* begin yangshuo: read |BS|..|ES| for semantic kernel */
//    printf("\nread |BS|...|ES| start, line=%s", &line[pos]);
    while (isspace((int)line[pos]) && line[pos] != 0) pos++;// go to "|BS|" marker or to the first number
    if (line[pos] != 0) read_semantic_vector_set(line + pos, doc, &pos);// read semantic kernel 
    else { doc->vectors = NULL; doc->num_of_vectors = 0; }
    //try print to verify
/*    if (doc->semanticVector != NULL)
    {
        for (int j = 0; j < doc->semanticVector->num_of_words; j++) {
            printf("ID=%d, len=%d, wordID=%s, len=%d, location=%s\n", j + 1, strlen(doc->semanticVector->words[j].wordID), doc->semanticVector->words[j].wordID,
                strlen(doc->semanticVector->words[j].location), doc->semanticVector->words[j].location);
        }
        printf("read |BS|...|ES| finished, num_of_words=%d, reading vectors=%s.\n", doc->semanticVector->num_of_words, &line[pos]); fflush(stdout);
    }*/
    /* end yangshuo: read |BS|..|ES| for semantic kernel */

    //printf("LINE:%s\n",line+pos);fflush(stdout);
    while (isspace((int)line[pos]) && line[pos] != 0) pos++;//remove spaces
    //while((!isspace((int)line[pos])) && line[pos]) pos++;
    // if vectors exists
    if (line[pos] != 0) read_vector_set(line + pos, doc, max_words_doc, totwords);// read the set of vectors 
    else { doc->vectors = NULL; doc->num_of_vectors = 0; }
...
//printf("Version file %s --- version label %s \n",version_buffer,VERSION);
  ...
  if(PARAM_VECT == 1) read_input_tree_kernel_param(/*tree_kernel_params yangshuo*/); // if there is the file tree_kernel.param load paramters
  ...
  for(i=1;i<model->sv_num;i++) {
    ...
    model->supvec[i]->semanticVector = NULL;//yangshuo 2020.10.11
    model->supvec[i]->docnum = 0;//yangshuo 2020.10.11
...
/* begin yangshuo */
    read_one_line(line, model->supvec[i], pos, max_words_doc, &fake_totwords, &model->kernel_parm);
#if 0
		    ...
		    long int *totwords, long int *totdoc, KERNEL_PARM *kernel_parm)
{
  ...
  if (kernel_parm->kernel_type < 4) { // from 0 to 3 are original kernels => no trees
      kernel_parm->second_kernel=kernel_parm->kernel_type; // *** yangshuo TK's bug 1 ***
  }
  ...
  while((!feof(docfl)) && fgets(line,(int)ll,docfl)) {
      ...
      if (dnum==0) {
          //yangshuo: this is the first line
          printf("\nfirst line, ignore the header, fill as space:[0x%x 0x%x 0x%x 0x%x 0x%x]\n", line[0], line[1], line[2], line[3], line[4]);
          line[0] = 0x20;
          line[1] = 0x20;
          line[2] = 0x20;
      }
    ...
    docs[dnum].semanticVector = doc.semanticVector; //yangshuo
...
//yangshuo ²âÊÔ´úÂë
  if (*label > 1.0f) {
      static double lastLabel = 0;
      double originLabel = *label;
      *label = -1.0f;
      if (lastLabel != originLabel) {
          printf("\nyangshuo test case: label=%lf,new label=%lf\n", originLabel, *label);
      }
      lastLabel = originLabel;
  }
  else {
      static double lastLabel2 = 0;
      if (lastLabel2 != *label) {
          printf("\nyangshuo test case: label=%lf, line=%s\n", *label, line+pos);
      }
      lastLabel2 = *label;
  }
  fflush(stdout);
...
/* begin yangshuo */
  read_one_line(line, doc, pos, max_words_doc, totwords, kernel_parm); 
#if 0
  read_tree_forest(line, doc, &pos);// read the tree forest: set PARAM_VECT to 0 if less paramaters than vector entries
  PARAM_VECT = kernel_parm->tree_kernel_params; // restore parameter file flag
...
//   printf("read_vector_set finished.\n");  fflush(stdout); 
   doc->docnum=-1;
 
  evaluateNorma(kernel_parm,doc);
  PARAM_VECT = kernel_parm->tree_kernel_params; // restore parameter file flag
/*  printf("parse_document finished.\n");   fflush(stdout); */
#endif
/* end yangshuo */
  return(1);
}
